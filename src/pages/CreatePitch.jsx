...
function CreatePitch() {
  ...
  // Fix HTML code to prevent navigation issues
  const fixHtmlCode = (html) => {
    ...
    let fixedHtml = html
      ...
      // Add onclick prevention to all links
      .replace(/<a /g, '<a onclick="return false;" ')
      
      // Add onclick prevention to all buttons
      .replace(/<button /g, '<button onclick="return false;" ')
      
      // Disable form submissions
      .replace(/<form /g, '<form onsubmit="return false;" ')
      
      // Insert base tag after head opening or create head if doesn't exist
      .replace(/<head>/, `<head>${baseTag}`);
    
    // If no head tag exists, wrap the content properly
    if (!fixedHtml.includes('<head>')) {
      fixedHtml = `<!DOCTYPE html><html><head>${baseTag}<style>
      ...
      </style></head><body>${fixedHtml}</body></html>`;
    }
...
IMPORTANT: For all links, use href="javascript:void(0)" instead of "#"
  ...
  const handleIframeLoad = () => {
    if (iframeRef.current) {
      try {
        ...
        console.log('Safety measures applied to iframe');
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Input Section - Moved to top and wider */}
        <div className="mb-12">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8 transition-all duration-300 hover:shadow-2xl max-w-4xl mx-auto">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                          Interactive Preview (Links Disabled)
                        </span>
                      </div>
                      <div className="border-2 border-gray-200 rounded-2xl overflow-hidden shadow-lg">
                        <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex items-center space-x-2">
                          ...
                          <span className="text-sm text-gray-600">Preview Only - Links Disabled</span>
                        </div>
                        <iframe
                          ref={iframeRef}
                          srcDoc={htmlCode}
